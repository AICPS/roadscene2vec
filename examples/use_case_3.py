...
def collison_pred():
    ...
    training_config = configuration(r"C:\Users\harsi\research\sg2vec\sg2vec\config\learning_config.yaml",from_function = True) #replace with path to sg2vec\config\learning_config.yaml
    ...
    output, _ = self.model.forward(model_input) #output collision predictions for the original sequence 
    return output
