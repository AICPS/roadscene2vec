...
def im2graph():
    real_scenegraph_extraction_config = configuration(r"use_case_2_scenegraph_extraction_config.yaml",from_function = True) #replace with path to roadscene2vec\config\data_config_real.yaml
...
def extract_seq():
    real_data_preprocessing_config = configuration(r"use_case_2_data_preprocessing_config.yaml",from_function = True) #replace with path to roadscene2vec\config\scene_graph_config_real.yaml
...
#     extracted_scenegraphs = configuration(r"C:\Users\harsi\research\roadscene2vec\roadscene2vec\config\scene_graph_config_real.yaml",from_function = True)
#     extracted_scenegraphs = SceneGraphDataset(extracted_scenegraphs)
#     extracted_scenegraphs = extracted_scenegraphs.load()
    training_config = configuration(r"use_case_2_learning_config.yaml",from_function = True) #replace with path to roadscene2vec\config\learning_config.yaml
                                                                                                                   #task_type in learning config training_configuration should be set to sequence_classification
                                                                                                                   #model_load_path should contain path to pretrained trainer in roadscene2vec/examples/pretrained_models
