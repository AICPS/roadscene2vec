...
from roadscene2vec.data import real_preprocessor as ip
...
from roadscene2vec.util.visualizer import *

def im2graph():
    real_scenegraph_extraction_config = configuration(r"use_case_1_scenegraph_extraction_config.yaml",from_function = True) #replace with path to roadscene2vec\config\data_config_real.yaml
...
def extract_seq():
    real_data_preprocessing_config = configuration(r"use_case_1_data_preprocessing_config.yaml",from_function = True) #replace with path to roadscene2vec\config\scene_graph_config_real.yaml
...
if __name__ == "__main__":
    extract_seq() 
    visualize(r"use_case_1_scenegraph_extraction_config.yaml") #visualize extracted scenegraphs using the stored location in the scene graph config